...
void *fill_memory_p(void *data) {
    if (!data) {
        printf("Wrong args in `fill_memory_p`: address: 0x%lx. Aborting.\n", (u_int64_t) data);
        pthread_exit((void *) 1);
    }
    __auto_type params = (write_threads_params_t *) data;
    __auto_type handler = fopen(kUrandomPath, "rb");
    ...
    __auto_type read_sz = fread(params->begin_addr, sizeof(char), params->size, handler);

    if (read_sz != params->size) {
        printf("Error on reading %li bytes from /dev/urandom. Aborting\n", params->size);
        goto aborting;
    }
...
bool fill_memory(void *address, size_t sz, size_t threads_n) {
    if (!address || !sz || !threads_n) {
        printf("Wrong args in `fill_memory`: address: 0x%lx, sz: %li, threads_n: %li. Aborting.\n", (u_int64_t) address,
               sz, threads_n);
        return false;
    }

    __auto_type threads = (pthread_t *) malloc(sizeof(pthread_t) * threads_n);
    ...
    for (size_t i = 0; i < threads_n; i++) {
        ...
        if (pthread_create(&threads[i], NULL, fill_memory_p, &threads_params[i])) {
            printf("Couldn't create thread n: %zu", i);
            goto aborting;
        }
    }
